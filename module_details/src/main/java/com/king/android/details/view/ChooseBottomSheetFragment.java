...
import google.architecture.coremodel.data.xlj.goodsdetail.ShopStock;
...
public class ChooseBottomSheetFragment extends BaseBottomSheetFrag {
    ...
    private SpecData mGettingSpec;//取件方式
                    ...
                    if(!TextUtil.isEmpty(catId)) {
                        if(catId.equals("1")) { //颜色
                            ...
                            if(selectedPosition <= specParams.specsSize.detailSpecInfos.size()) {
                                ...
                                updateDeliverSelect(specParams.specsSize.detailSpecInfos.get(selectedPosition), mGettingSpec);
    ...
    private List<SpecData> prepareData(SpecParams specParams) {
        ...
        initDeliverStyle(specsSize.detailSpecInfos, gettingSpec);
        mGettingSpec = gettingSpec;
        return specDatas;
    }

    /**
     * 初始化取件方式
     * @param detailSpecInfos 尺码
     */
    public void initDeliverStyle(List<google.architecture.coremodel.data.xlj.goodsdetail.List> detailSpecInfos, SpecData gettingSpec){
        for(int i = 0; i < detailSpecInfos.size(); i++){
            if(detailSpecInfos.get(i).getIsDefault() == 1){
                updateDeliverSelect(detailSpecInfos.get(i),gettingSpec);
                break;
            }
        }
    }

    public void updateDeliverSelect(google.architecture.coremodel.data.xlj.goodsdetail.List item, SpecData gettingSpec){
        ShopStock s = new ShopStock();
        if((item.getShopStock() != null && item.getShopStock().getAllStock() > 0)){//
            gettingSpec.item1.get(0).setIsDefault(1);
            gettingSpec.item1.get(1).setIsDefault(0);
        }else if(item.getShopStock() != null && item.getShopStock().getUnStock() > 0){
            gettingSpec.item1.get(0).setIsDefault(0);
            gettingSpec.item1.get(1).setIsDefault(1);
        }
        if((item.getShopStock() != null && item.getShopStock().getAllStock() > 0)){
            s.setAllStock(1);
            gettingSpec.item1.get(0).setShopStock(s);
        }
        if(item.getShopStock() != null && item.getShopStock().getUnStock() > 0){
            s.setUnStock(1);
            gettingSpec.item1.get(1).setShopStock(s);
        }
    }
