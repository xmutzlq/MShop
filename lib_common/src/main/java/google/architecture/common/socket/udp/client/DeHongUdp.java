package google.architecture.common.socket.udp.client;
...
import google.architecture.common.socket.udp.client.manager.UdpSocketManager;
 ...
 * udp 客户端
 */
public class DeHongUdp extends BaseDeHongSocket {
    private static final String TAG = "DeHongUdp";
    ...
    public static DeHongUdp getUdpClient() {
        DeHongUdp client = new DeHongUdp();
        client.init();
        return client;
    }

    private void init() {
        mUdpClientListeners = new ArrayList<UdpClientListener>();
    ...
    public void startUdpServer() {
        if (!getReceiveThread().isAlive()) {
            ...
            DeHongSocketLog.d(TAG, "udp server started");
        }
    }
        ...
        public void run() {
            ...
            try {
                while (!Thread.interrupted()
                    ...
                    DeHongSocketLog.d(TAG, "udp send msg=" + msg);
        ...
        public void run() {
            ...
            while (!Thread.interrupted()) {
                try {
                    ...
                    DeHongSocketLog.d(TAG, "udp receive byte=" + Arrays.toString(res));
                    ...
                    DeHongSocketLog.d(TAG, "udp receive msg=" + udpMsg);
        ...
        for (UdpClientListener l : mUdpClientListeners) {
            ...
            if (listener != null) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        listener.onReceive(DeHongUdp.this, msg);
                    }
                });
            }
        }
        ...
        for (UdpClientListener l : mUdpClientListeners) {
            ...
            if (listener != null) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        listener.onStarted(DeHongUdp.this);
                    }
                });
            }
        }
        ...
        for (UdpClientListener l : mUdpClientListeners) {
            ...
            if (listener != null) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        listener.onStoped(DeHongUdp.this);
                    }
                });
            }
        }
        ...
        for (UdpClientListener l : mUdpClientListeners) {
            ...
            if (listener != null) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        listener.onSended(DeHongUdp.this, msg);
                    }
                });
            }
        }
        ...
        for (UdpClientListener l : mUdpClientListeners) {
            ...
            if (listener != null) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        listener.onError(DeHongUdp.this, msg, e);
                    }
                });
            }
        }
